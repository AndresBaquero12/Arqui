...
export function ChessGame({ onBack, playerName = "Jugador", playerId, gameId, gameMode, preferredColor = 'white', difficulty = 'medium' }: ChessGameProps) {
  ...
  const { board, currentTurn, whiteTime, blackTime, moves, whiteCaptures, blackCaptures, selectedSquare, legalMovesForSelected, kingInCheck, gameOverMessage } = state;
  const whitePieces = ['♔', '♕', '♖', '♗', '♘', '♙'];
      ...
      <div className="relative z-10 min-h-screen p-4 md:p-8">
        ...
        {gameOverMessage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="bg-slate-800 border border-slate-600 rounded-2xl p-8 shadow-2xl max-w-md mx-4 text-center animate-in fade-in zoom-in duration-300">
              <div className="text-6xl mb-4">
                {gameOverMessage.includes('ganado') ? '🏆' : gameOverMessage.includes('perdido') ? '😞' : '🤝'}
              </div>
              <h2 className="text-2xl font-bold text-white mb-2">Fin de la Partida</h2>
              <p className="text-lg text-slate-300 mb-6">{gameOverMessage}</p>
              <div className="flex gap-3 justify-center">
                <Button
                  variant="outline"
                  onClick={() => { actions.clearGameOver(); actions.handleRestart(); }}
