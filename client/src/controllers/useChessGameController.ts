...
export function useChessGameController(
  ...
  const checkGameOver = () => {
    ...
    if (chess.isCheckmate()) {
      const loser = chess.turn(); // el que tiene que mover está en mate
      const winnerColor = loser === 'w' ? 'Negras' : 'Blancas';
      let message: string;
      let reason: string;
      if ((loser === 'w' && playerColor === 'white') || (loser === 'b' && playerColor === 'black')) {
        message = `¡Jaque Mate! Has perdido. Ganan las ${winnerColor}.`;
        reason = 'Derrota por Jaque Mate';
      } else {
        message = `¡Jaque Mate! ¡Has ganado! Ganan las ${winnerColor}.`;
        reason = 'Victoria por Jaque Mate';
      }
      ...
      setGameOverMessage('¡Tablas por ahogado! Ningún jugador gana.');
      storageModel.saveGameToHistory({
        id: gameId,
        date: new Date().toISOString(),
        playerName,
      ...
      setGameOverMessage('¡Tablas! La partida terminó en empate.');
      storageModel.saveGameToHistory({
        id: gameId,
        date: new Date().toISOString(),
        playerName,
  ...
  const handleSquareClick = (row: number, col: number) => {
    ...
    if (gameMode === 'multiplayer' || gameMode === 'fast' || gameMode === 'standard') {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ action: 'move', payload: { from, to, promotion: 'q' } }));
      }
    }
  };
      ...
      try {
        ...
        if (message.type === 'gameOver') {
          ...
          if (winner === playerColor) {
            setGameOverMessage('¡Jaque Mate! ¡Has ganado!');
          } else if (winner) {
            setGameOverMessage(`¡Jaque Mate! Has perdido. Ganan las ${winner === 'white' ? 'Blancas' : 'Negras'}.`);
          } else {
            setGameOverMessage('¡La partida ha terminado!');
          }
          return;
        }
        if (message.type === 'eloUpdate') {
          console.log('ELO actualizado:', message.payload);
          return;
        }
  ...
  return {
    ...
    actions: {
      ...
      clearGameOver: () => setGameOverMessage(null),
      setRoomCodeInput,
      handleCreateRoom,
      handleJoinRoom,
      handleCopyRoomCode
